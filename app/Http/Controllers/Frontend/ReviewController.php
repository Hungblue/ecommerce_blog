...
class ReviewController extends Controller
{
    public function add($product_slug)
    {
      ...
      if($product)
      {
        ...
        $review = Review::where('user_id', Auth::id())->where('status', '1')->first();
        if($review)
        {
        ...
        return view('frontend.reviews.index', compact('product', 'verified_purchase', 'featured_categories'));
        }
      }
      else
      {
    ...
    public function edit($product_slug)
    {
      ...
      if($product)
      {
        ...
        if($review)
        {
          return view('frontend.reviews.edit', compact('review', 'featured_categories'));
        }
        else
        {
          return redirect()->back()->with('status', "The link you followed was broken");
